...
possChans = project_utils.corrChanSelZeroMeanDarpa_jag(X,Z,1:size(X,1),corrKineThresh,size(Z,1),windowType);
    ...
    for j = 1:min(maxChans,numel(possChans))
        ...
        for k = 1:size(resid,1) %find stopping criterion value
            if ~strcmp(stopCrit,'none')
                loss{i}(k,j) = project_utils.stopCritDarpa([],[],resid(k,:),j-1,stopCrit);          
            else
                loss{i}(k,j) = -j;
            end
        ...
        % corcoef() to find correlation and get p-value instead of above
        % line
        corrs(isnan(corrs)) = 0;
