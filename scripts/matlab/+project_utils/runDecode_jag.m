function [xhat] = runDecode(TRAIN, TestX, TestZ, Idxs, method, varargin)
% inputs
...
KalmanGain = ones(size(TRAIN.A, 1), 2); % This generalizes to more DOFs than the original
...
for j = 1: size(TestX,2)
    switch method
        case 'standard'
            xhat(:,j) = project_utils.kalman_test(TestZ(Idxs,j),TRAIN,[-1./KalmanGain(:,2),1./KalmanGain(:,1)],j==1);
        case 'DWPRR'
            ...
            xhat(:,j) = project_utils.kalman_test(tempFeat,TRAIN,[-1./KalmanGain(:,2),1./KalmanGain(:,1)],j==1);
        case 'KalmanSS'
            xhat(:,j) = project_utils.imprtKalman_testSS(TestZ(Idxs,j),TRAIN,[-1./KalmanGain(:,2),1./KalmanGain(:,1)],j==1);
    end
    ...
    % correct zero crossings due to gain-threshold transformation
