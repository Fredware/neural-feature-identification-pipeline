...
switch method
    case 'standard'
        TRAIN = project_utils.kalman_train(TrainX,TrainZ(Idxs,:));
        %             xhat = zeros(length(KalmanMvnts), size(TestX,2));
        a = project_utils.kalman_test(TrainZ(Idxs,1),TRAIN,[-1./KalmanGain(:,2),1./KalmanGain(:,1)],1);% init
    case 'DWPRR'
        ...
        TRAIN = project_utils.kalman_train(TrainX,trainEst);
        
        %             ' = zeros(length(KalmanMvnts), size(TestX,2));
        a = project_utils.kalman_test(trainEst(:,1),TRAIN,[-1./KalmanGain(:,2),1./KalmanGain(:,1)],1);% init
    case 'KalmanSS'
        TRAIN = project_utils.kalman_trainSS(TrainX,TrainZ(Idxs,:));
    case 'third-order'
        TRAIN = project_utils.kalman3_pm_train(TrainX, TrainZ(Idxs, :));
end